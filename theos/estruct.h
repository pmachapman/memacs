...
#if	MSC || TURBO || IC || VMS || GCC || ZTC || THEOSC
#define PROTO	1
#else
#define PROTO	0
#endif
...
#if	(__STDC__ || MSC || TURBO || IC || ZTC) && ! THEOSC
#define ETYPE	union
#else
#define ETYPE	struct
#endif
...
#if	MSDOS | WINNT | OS2 | USG | LINUX | AIX | AUX | SMOS | HPUX8 | HPUX9 | BSD | FREEBSD | (TOS & MWC) | WMCS | SUN | MPE | THEOS
#define ENVFUNC 1
#else
#define ENVFUNC 0
#endif
...
#if	AOSVS | VMS | WINNT | SUN | BSD | FREEBSD | USG | LINUX | ZENIX | HPUX8 | HPUX9 | OS2 | THEOS
#define NFILEN	256
#else
...
typedef struct	BUFFER {
	...
	int	b_exec; 		/* how many active executions	*/
	int	b_flag; 		/* Flags			*/
#else
