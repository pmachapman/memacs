...
void ttopen()
{
...
#endif
#if	GCC
	/* register an exit function in case we don't exit nicely */
	atexit(ttclose);
#endif
	...
	AslBase = (struct Library *)
		OpenLibrary("asl.library", ASL_REV);
	if (AslBase == NULL) {
                if (GfxBase) CloseLibrary((struct Library *)GfxBase);
 ...
 * we just put the characters in the big array, checking for overflow.
 */
...
/*
 * Flush terminal buffer. Uses sendcon() to do the real work.
 */
...
/* stuff a key in the input buffer */
...
int FileReq(char *rtitle, char *filename, unsigned maxlen)
{
