...
#define JMDEXT	1	/* enable J-M Dubois' extensions		*/
...
/*	[Set one of these!!]		*/
#define ASCII	0	/* use ASCII char sequences			*/
#define EBCDIC	0	/* later IBM mainfraim versions will use EBCDIC */
#define UTF8	1	/* use UTF-8 encoded Unicode			*/
...
#define MOUS	0x10000000	/* alternative input device (mouse)	*/
#define SHFT	0x20000000	/* shifted (for function keys)		*/
#define ALTD	0x40000000	/* ALT key...				*/
#define MENU	MOUS+SPEC	/* menu selection (WINDOW_MSWIN)	*/
#define CMSK	0x00FFFFFF	/* Character mask			*/
#define SHIFTPFX 24		/* prefix shift in unsigned int		*/
#else
...
#define MENU	MOUS+SPEC	/* menu selection (WINDOW_MSWIN)	*/
#define CMSK	0x00FF		/* Character mask			*/
#define SHIFTPFX 8		/* prefix shift in unsigned int		*/
#endif
...
#endif
#if	UTF8
	unsigned int	v_text[1];		/* Screen data. */
#else
...
#define VFREV   0x0008			/* reverse video status 	*/
#define VFREQ   0x0010			/* reverse video request 	*/
#define VFCOL	0x0020			/* color change requested	*/
...
typedef struct	BUFFER {
	...
	int	b_fcol; 		/* first column to display 	*/
...
#define MDSLINE	0x1000			/* single line graphics		*/
#define MDDLINE	0x2000			/* double line graphics		*/
#if	THEOX
#define MDTHEOX 0x4000			/* TheoX character set file	*/
#endif
#endif
...
#endif
#if     WINDOW_MSWIN
        int (*t_sleep)(int);   /* go to sleep for a while	*/
        int (*t_newscr)(ESCREEN *);  /* create new screen display */
        int (*t_delscr)(ESCREEN *);  /* destroy screen display */
        int (*t_selscr)(ESCREEN *);  /* select screen display */
        int (*t_sizscr)(ESCREEN *);  /* resize screen display */
        int (*t_topscr)(ESCREEN *);  /* bring screen to top	*/
#endif
}	TERM;
...
typedef struct	{
	...
	int (*t_clrdesk)(); /* Clear the page totally	*/
	int (*t_beep)();	/* Beep.			*/
...
#endif
#if	INSDEL
	int (*t_insline)(); /* insert a screen line 	*/
	int (*t_delline)(); /* delete a screen line 	*/
#endif
#if     WINDOW_MSWIN
        int (*t_sleep)();   /* go to sleep for a while	*/
        int (*t_newscr)();  /* create new screen display	*/
        int (*t_delscr)();  /* destroy screen display	*/
        int (*t_selscr)();  /* select screen display	*/
        int (*t_sizscr)();  /* resize screen display	*/
        int (*t_topscr)();  /* bring screen to top		*/
#endif
}	TERM;
#endif
